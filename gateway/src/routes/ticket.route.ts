...
ticket
  .get("/:id", checkRoles(["admin", "user"]), async (c) => {
    ...
    const data = await helpdeskApi.post(`tickets/${id}`).json<Record<string, unknown>>()
    return c.json(data)
  })
  .put("/:id", checkRoles(["admin"]), async (c) => {
    const id = c.req.param("id")
    const body = await c.req.json()
    const data = await helpdeskApi.post(`tickets/${id}`, { json: body }).json<Record<string, unknown>>()
    return c.json(data)
  })
